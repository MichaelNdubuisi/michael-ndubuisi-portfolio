...
export default function TechStack() {
  return (
    <section id="stack" className="bg-primary text-text px-6 py-20">
      <div className="max-w-5xl mx-auto space-y-12 text-center">
        <div className="fade-in-down">
          <h2 className="text-3xl sm:text-4xl font-bold">Tech Stack</h2>
          <p className="text-gray-400 mt-2">Tools and technologies I use to build clean, fast websites</p>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-8 fade-in-up">
          {tech.map((item, index) => (
            <div
              key={index}
              className="bg-[#101e3b] p-6 rounded-xl flex flex-col items-center justify-center space-y-3 scale-in"
            >
              {item.icon}
